...
_strut() {
  ...
  top_cmds=(init list scaffold upgrade doctor status-all dashboard posture group monitoring audit audit:list audit:diff audit:generate migrate migrate:status notify sync fleet webhook secrets-filter mcp skills help completions remote:init gateway --version --help)
  ...
  case "$cmd" in
    backup)
      ...
      ;;
    drift)
      [ "$pos" -eq 4 ] && { compadd detect report diff fix monitor history images auto-fix; return 0 }
      ;;
    db:pull|db:push)
    ...
    lock)
      ...
      ;;
    first-run)
      [ "$pos" -eq 4 ] && { compadd --status --force; return 0 }
      ;;
