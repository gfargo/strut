...
keys_ssh_rotate() {
  ...
  # Capture the fingerprint of the key being replaced BEFORE keys_ssh_add
  # runs — keys_ssh_add overwrites the "username" metadata entry with the
  # NEW key's fingerprint/key_file, so this is the only point at which the
  # OLD key's fingerprint is still recoverable. Empty if this is the user's
  # first-ever key (nothing to revoke afterward).
  local old_fingerprint
  old_fingerprint=$(jq -r --arg username "$username" '.ssh_keys[] | select(.username == $username) | .fingerprint' "$metadata_file")
  ...
  new_fingerprint=$(jq -r --arg username "$username" '.ssh_keys[] | select(.username == $username) | .fingerprint' "$metadata_file")

  # Remove the OLD physical key from the VPS. Deliberately NOT a call to
  # keys_ssh_revoke: that command deletes the metadata entry for $username,
  # which by now correctly describes the NEW key added above — routing
  # through it here would discard the rotation's own result. Skip if there
  # was no prior key (first-ever add for this user).
  if [ -n "$old_fingerprint" ] && [ "$old_fingerprint" != "null" ] && [ "$old_fingerprint" != "$new_fingerprint" ]; then
    log "Revoking previous key for $username..."
    local removed_count
    removed_count=$(_keys_ssh_revoke_fingerprint "$stack" "$env_file" "$old_fingerprint") ||
      fail "Rotation left both keys active: failed to revoke the old key for $username (fingerprint: $old_fingerprint)"
    if [ "$removed_count" = "missing" ] || [ -z "$removed_count" ] || [ "$removed_count" -eq 0 ] 2>/dev/null; then
      warn "Could not find the previous key on the VPS to revoke — it may already be gone"
    else
      ok "Previous key revoked"
    fi
    log_key_operation "$stack" "ssh:rotate" "Rotated SSH key for $username (old fingerprint: $old_fingerprint, new: $new_fingerprint)"
  else
    log_key_operation "$stack" "ssh:rotate" "Added first SSH key for $username (fingerprint: $new_fingerprint)"
  fi

  ok "SSH key rotated for $username"
}

# _keys_ssh_revoke_fingerprint <stack> <env_file> <fingerprint>
#
# Low-level primitive: removes every authorized_keys entry on the VPS whose
# fingerprint matches. Echoes the number of entries removed, or "missing" if
# there's no authorized_keys file at all. Does NOT touch metadata, does NOT
# run the last-admin-key guard, does NOT prompt for confirmation — callers
# (keys_ssh_revoke, keys_ssh_rotate) own those concerns, since they apply
# differently to a plain revoke vs. the old-key cleanup half of a rotation.
_keys_ssh_revoke_fingerprint() {
  local stack="$1"
  local env_file="$2"
  local fingerprint="$3"
  ...
  # Remove ONLY the key whose fingerprint matches. Two earlier approaches
  # were both wrong:
  #   - `grep -v "$username"` matched the username as a substring of the
  #     WHOLE line (incl. the base64 blob and the "ssh-ed25519" type), so
  #     revoking a short/common username could strip every key = lockout.
  #   - `awk '$NF == username'` (the fix for the above) matched the trailing
  #     comment field against the bare username, but keys generated by
  #     ssh:add carry comment "username@stack" (see ssh-keygen -C above), and
  #     keys added via --key-file can carry any comment at all — so the
  #     comment almost never equals the bare username and revoke silently
  #     removed nothing, reporting success.
  #
  # Fingerprint identifies exactly one keypair regardless of its comment, and
  # (unlike the comment) is guaranteed to differ between the old and new key
  # during a rotation, so it can't remove both.
  local safe_fp
  safe_fp=$(printf '%q' "$fingerprint")
  local revoke_result
  revoke_result=$(ssh $ssh_opts "$vps_user@$vps_host" "
    if [ ! -f ~/.ssh/authorized_keys ]; then
      echo 'STRUT_REVOKE_RESULT:missing'
      exit 0
    fi
    ...
    while IFS= read -r line || [ -n \"\$line\" ]; do
      case \"\$line\" in
        ''|'#'*)
          printf '%s\n' \"\$line\" >> ~/.ssh/authorized_keys.tmp
          continue
          ;;
      esac
      this_fp=\$(printf '%s\n' \"\$line\" | ssh-keygen -lf /dev/stdin 2>/dev/null | awk '{print \$2}')
      if [ \"\$this_fp\" = $safe_fp ]; then
        removed=\$((removed + 1))
        continue
      fi
      printf '%s\n' \"\$line\" >> ~/.ssh/authorized_keys.tmp
    ...
    mv ~/.ssh/authorized_keys.tmp ~/.ssh/authorized_keys
    chmod 600 ~/.ssh/authorized_keys
    echo \"STRUT_REVOKE_RESULT:\$removed\"
  ")

  printf '%s\n' "$revoke_result" | grep -o 'STRUT_REVOKE_RESULT:.*$' | cut -d: -f2
}
...
keys_ssh_revoke() {
  ...
  local removed_count
  removed_count=$(_keys_ssh_revoke_fingerprint "$stack" "$env_file" "$fingerprint") || return 1

  if [ "$removed_count" = "missing" ]; then
    warn "No ~/.ssh/authorized_keys on VPS — nothing to revoke"
    return 1
  fi

  if [ -z "$removed_count" ] || [ "$removed_count" -eq 0 ] 2>/dev/null; then
    fail "Revoke failed: no authorized_keys entry matched fingerprint $fingerprint for $username. Nothing was changed on the VPS."
  fi

  if [ "$removed_count" -gt 1 ]; then
    warn "Removed $removed_count matching entries for $username (duplicate keys on VPS)"
  fi
