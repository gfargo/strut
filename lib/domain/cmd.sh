#!/usr/bin/env bash
# ==================================================
# lib/domain/cmd.sh — Domain/SSL subsystem command
# ==================================================
# Full logic for `strut <stack> domain <domain> <email> [--skip-ssl]`.
# Called from lib/cmd_domain.sh thin-router.

set -euo pipefail

# domain_command <stack> <env_file> <env_name> [args...]
domain_command() {
  local stack="$1"
  local env_file="$2"
  local env_name="$3"
