...
_usage_gen() {
  ...
  echo ""
  echo "Applied automatically at deploy time as the final env layer — a"
  echo "generated value overrides the base env file, the tracked per-host"
  echo "layer, and decrypted secrets on a conflicting key (host-scope wins"
  echo "over stack-scope). See env_apply_gen_layer in lib/utils.sh."
  echo ""
  echo "Examples:"
